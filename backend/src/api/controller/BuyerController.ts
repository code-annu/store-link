...
export class BuyerController {
    ...
    constructor(buyerRepo: IBuyerRepository, userRepo: IUserRepository) {
        this.createBuyerProfile = new CreateBuyerProfileUsecase(buyerRepo, userRepo)
        this.getBuyerProfile = new GetBuyerProfileUsecase(buyerRepo)
        this.deleteBuyerProfile = new DeleteBuyerProfileUsecase(buyerRepo,userRepo)
    ...
    async postBuyer(req: AuthRequest, res: Response, next: NextFunction) {
        try {
            ...
            const result = await this.createBuyerProfile.execute(userUid!, {
                fullname: fullname,
                address: address,
    ...
    async patchBuyer(req: AuthRequest, res: Response, next: NextFunction) {
        try {
            const userUid = req.auth?.userId
            const data = req.body
            const result = await this.updateBuyerProfile.execute(userUid!, data)
            res.status(200).json(result)
        } catch (error) {
            next(error)
        }
    ...
    async deleteBuyer(req: AuthRequest, res: Response, next: NextFunction) {
        try {
            ...
            res.status(200).json({message: 'Buyer deleted successfully', user: result})
        } catch (error) {
            next(error)
        }
    }
